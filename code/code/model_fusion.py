...
class CommentGenerator_fusion(nn.Module):
    def __init__(self):
        ...
        model_path = "/Users/jetzel/Documents/Grad/CSE_575/BART-fusion/BART-fusion/code/model/best_model.pt"

        #self.music_encoder = CNNSA().cuda()

        self.device = torch.device(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
<<<<<<< HEAD
        self.music_encoder = CNNSA().to(self.device)
=======
        self.music_encoder = CNNSA().to(device)
>>>>>>> 8468cab (cuda/cpu)
    ...
    def forward(self, input_sentence_list, music_ids, labels=None):
        ...
        output = self.bart(input_ids=encoded_input['input_ids'].to(self.device)),
    ...
    def generate(self, input_sentence_list, music_ids, is_cuda=True):
        ...
        output_ids = self.bart.generate(encoded_input['input_ids'].to(self.device),
                                        num_beams=5,
                                        max_length=512,
                                        early_stopping=True,
                                        do_sample=True,
